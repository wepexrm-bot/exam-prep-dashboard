...
Render auto-deploys on every push to `main`. The mobile APK loads from this live URL via WebView — no local build pipeline needed for feature updates. Only rebuild the APK when native Capacitor plugins change.
