          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              ...
              {selectedDeadlines.map((g, idx) => {
                const tc = TAG_COLORS[g.tag] || TAG_COLORS.Other;
                const isFuture = selectedDate != null && selectedDate > todayK;
                const isPast = selectedDate != null && selectedDate < todayK;
                return (
                  <div key={`deadline-${g.id}`} style={{
                    ...
                    <button
                      onClick={() => { if (isFuture || isPast) return showToast(isFuture ? 'Cannot check off goals on a future date' : 'Cannot modify goals on a past date'); toggleGoal(g.id); }}
                      style={{
                        width: 20, height: 20, borderRadius: 6, flexShrink: 0, cursor: isFuture || isPast ? 'not-allowed' : 'pointer',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        background: g.done ? '#4ADE80' : 'transparent',
                        border: g.done ? 'none' : '1.5px solid rgba(255,255,255,0.2)',
                        boxShadow: g.done ? '0 0 8px rgba(74,222,128,0.5)' : 'none',
                    ...
                    >{g.done && I.check}</button>
                    <span style={{ color: g.done ? '#4ADE80' : '#F87171', flexShrink: 0 }}>{g.done ? I.check : I.flag}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 700, padding: '3px 9px', borderRadius: 99, flexShrink: 0, background: tc.bg, color: tc.color }}>{g.tag}</span>
                    <button
                      onClick={() => { if (isPast) return showToast('Cannot modify goals on a past date'); deleteGoal(g.id); }}
                      style={{ background: 'none', border: 'none', color: isPast ? 'rgba(255,255,255,0.15)' : 'var(--muted)', cursor: isPast ? 'not-allowed' : 'pointer', padding: '2px 4px', flexShrink: 0, display: 'flex' }}
                    >{I.trash}</button>
                  </div>
                );
              })}
